...
/**
 * Strings for component 'grades', language 'ru', branch 'MOODLE_22_STABLE'
 *
 * @package   grades
...
$string['aggregateonlygraded_help'] = 'Пустая оценка, это оценка, которая отсутствует в журнале оценок. Это может произойти, например, из-за того, что задание ещё не было оценено или студент ещё не приступал к тесту и т.п.

Этот параметр определят, нужно ли учитывать пустые оценки при расчёте итоговой. В том случае, если пустые оценки учитываются, вместо пустого значения используется наименьшее возможное значение - так, например, для задания, оцениваемого от 0 до 100 будет выбрано значение 0.';
$string['aggregateoutcomes'] = 'Включать оценки за показатели в итоговую оценку';
$string['aggregateoutcomes_help'] = 'Если параметр включен, оценки по отдельным показателям включаются в расчёт итоговой оценки. Это может привести к непредсказуемому результату в категории.';
$string['aggregatesonly'] = 'Только итоги';
$string['aggregatesubcats'] = 'Учитывать оценки из подкатегорий при расчёте итоговой';
$string['aggregatesubcats_help'] = 'Этот параметр определяет, используются ли оценки из подкатегорий при расчёте итога.';
...
Если итоговая оценка рассчитывается как "Среднее оценок (с учетом доп. баллов)" и значение параметра "Дополнительный балл / вес" больше нуля, то оценка выступает в роли дополнительного балла при расчёте итога. "Дополнительный балл / вес" - это множитель на который оценка будет умножаться перед тем как сложить все оценки при вычислении среднего.';
...
$string['aggregationcoefextraweight_help'] = 'Если значение параметра "Вес дополнительного балла" больше нуля, то оценка выступает в роли дополнительного балла при расчёте итога. "Вес дополнительного балла" - это множитель на который оценка будет умножаться перед тем как сложить все оценки при вычислении среднего.';
$string['aggregationcoefweight'] = 'Вес элемента';
$string['aggregationcoefweight_help'] = 'Вес элемента используется для расчёта итога по категории, чтобы указать важность одних элементов в категории по сравнению с другими.';
...
* Сумма оценок - Сумма значений оценок без учёта оценок, выставленных по шкалам.';
...
$string['aggregationsvisible'] = 'Доступные способы расчёта итоговой оценки';
$string['aggregationsvisiblehelp'] = 'Выберите все способы расчёта итоговой оценки, которые должын быть доступны. Для выбора нескольких элементов нажмите и удерживайте клавишу Ctrl.';
...
$string['coursegradedisplayupdated'] = 'Формат представления оценок курса был обновлён';
...
$string['csv'] = 'CSV';
$string['currentparentaggregation'] = 'Текущий способ расчёта итоговой оценки в родительской категории';
$string['curveto'] = 'Привести к';
$string['decimalpoints'] = 'Знаков после запятой';
$string['decimalpoints_help'] = 'Этот параметр определяет количество десятичных знаков при отображении каждой оценки. Это не влияет на вычисляемые оценки, которые рассчитываются с точностью до 5 десятичных знаков.';
...
$string['droplow_help'] = 'Этот параметр позволяет не учитывать указанное число самых низких оценок при расчёте итоговой оценки.';
...
$string['errorupdatinggradecategoryaggregatesubcats'] = 'Ошибка при обновлении параметра "Учитывать оценки из подкатегорий при расчёте итоговой" категории с ID {$a->id}';
$string['errorupdatinggradecategoryaggregation'] = 'Ошибка обновления способа расчёта итоговой оценки для категории с ID {$a->id}';
...
$string['excluded_help'] = 'Если отмечено, оценка не будет использоваться при расчёте итоговых оценок';
...
$string['extracreditwarning'] = 'Примечание: установка всех элементов категории в качестве дополнительных баллов исключит их из подсчёта оценки. Следовательно, в этом случае вообще не будет итоговой оценки.';
...
$string['gradedisplaytype_help'] = 'Этот параметр определяет как оценки отображаются в Журнале оценок и пользователям отчетов.

* Значение - фактические оценки
* Процент
* Буква - буквы или слова используются для представления диапазонов оценок';
...
$string['gradeexportdisplaytype_desc'] = 'Оценки при экспорте могут быть выведены как реальные оценки, в процентах (по отношению к минимальной и максимальной оценке) или как буквы (A, B, C и т.д..). Этот параметр может быть переопределён при экспорте.';
...
Для расчёта итоговой оценки могут использоваться только оценки типа "Значение" и "Шкала". Для оценок, связанных с элементами курса, тип устанавливается на странице настройки элемента курса.';
...
$string['hidetotalifhiddenitems_help'] = 'Этот параметр определяет, будут ли итоги, зависящие от скрытых оценок, отображаться студентам или будут заменяться дефисом (-). Если будут отображаться, то итоговая оценка может быть рассчитана либо с учётом, либо без учёта скрытых элементов.

Если скрытые элементы будут исключены из расчёта итоговой оценки, то оценка, отображаемая преподавателю в журнале, будет отличаться от отображаемой студенту, так как преподаватель всегда видит итоги, рассчитываемые из всех элементов, скрытых или не скрытых. Если скрытые элементы включаются в расчёт, то студенты, возможно, смогут вычислить значения скрытых элементов.';
$string['hidetotalshowexhiddenitems'] = 'Выводить итоги без учёта скрытых элементов';
$string['hidetotalshowinchiddenitems'] = 'Выводить итоги с учётом скрытых элементов';
...
$string['includescalesinaggregation_help'] = 'Вы можете выбрать, включать ли оценки по шкале (в виде чисел) во все итоговые оценки во всех журналах оценок во всех курсах. ВНИМАНИЕ: изменение этого параметра приведёт к пересчёту всех итоговых оценок.';
...
$string['meanselection_help'] = 'Этот параметр определяет, будет ли пустая ячейка без оценки включена в расчёт среднего значения оценки для всех категорий или элементов оценивания.';
...
$string['nocourses'] = 'Пока ещё нет ни одного курса';
...
$string['outcomereport'] = 'Отчёт по показателям';
...
$string['profilereport_help'] = 'Отчёт об оценках, используемый на странице профиля';
$string['publishing'] = 'Публикация';
$string['quickfeedback'] = 'Быстрый отзыв';
$string['quickgrading'] = 'Режим быстрой оценки';
$string['quickgrading_help'] = 'Если включен режим быстрой оценки, то при редактировании рядом с каждой оценкой отображается поле ввода текста, что позволяет одновременно редактировать несколько оценок. Изменения сохраняются при нажатии кнопки Обновить.
...
$string['reportplugins'] = 'Плагины отчётов';
...
$string['showeyecons_help'] = 'Если включено, то при редактировании у каждой оценки отображается значок "показать/скрыть" для управления её отображением для студента.';
...
$string['warningexcludedsum'] = 'Внимание: исключение оценок из расчёта итоговой оценки не совместимо с итоговой оценкой типа "Сумма оценок".';
$string['weight'] = 'вес';
