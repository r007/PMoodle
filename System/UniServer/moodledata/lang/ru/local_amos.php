...
/**
 * Strings for component 'local_amos', language 'ru', branch 'MOODLE_22_STABLE'
 *
 * @package   local_amos
...
$string['amos:importfile'] = 'Импортировать строки из загруженного файла';
...
$string['contribactions_help'] = 'В зависимости от Ваших прав и организации процесса приёма вклада в перевод, Вам будут доступны некоторые из следующих действий:

* Применить - скопировать внесённый перевод в свой этап и не изменять рассматриваемую запись.
...
* Начать рассмотрение - назначить себя ответственным, установить статус "На рассмотрении" и скопировать внесённый перевод в свой этап.
* Принять - отметить вклад как принятый.
* Отклонить - отметить вклад как отклонённый. Пожалуйста, укажите причины в комментарии.

Внёсший вклад пользователь информируется по электронной почте при любых изменениях статуса вклада.';
...
$string['contribstagedinfo_help'] = 'Этап содержит строки, являющиеся присланным членом сообщества вкладом в перевод. Предполагается, что ответственный за языковой пакет просмотрит их, после чего установит соответствующий статус вклада: "Принят" (если строки были зафиксированы) или "Отклонён" (если по какой-то причине строки не могут быть включены в официальный языковой пакет).';
...
$string['greylisted_help'] = 'На всякий случай, языковой пакет Moodle может содержать строки, которые больше не используются, но ещё не были удалены. Эти строки занесены в "серый список". Как только будет подтверждено, что сроки из серого списка более не используются, они будут удалены из языкового пакета.

Если Вы обнаружили, что  серый список попала строка, которая ещё используется в Moodle, сообщите нам об этом в сообщении на форуме в курсе "Translating Moodle" на этом сайте. В противном случае, вы можете сэкономить драгоценное время, переводя строки, которые скорее всего используются и пропускать строки из серого списка.';
$string['greylistedwarning'] = 'строка из "серого списка"';
$string['importfile'] = 'Импортировать переведённые строки из файла';
$string['importfile_help'] = 'Если у Вас есть строки, переведённые оффлайн, Вы можете загрузить их в текущий этап с помощью этой формы.
...
$string['stashautosave_help'] = 'Эта копилка содержит наиболее свежий снимок Вашего этапа. Её можно использовать, например, как резервную копию, в том случае, когда все строки были случайно очищены.
...
* Зелёный - строка уже переведена и Вам разрешено изменить перевод и зафиксировать его.
* Жёлтый - Строку можно зафиксировать, но, возможно, она устарела. Английская версия строки могла измениться после перевода.
* Красный - Строка не переведена, и Вам разрешено перевести её и зафиксировать перевод.
* Голубой - Вы изменили перевод и он сохранён в текущем этапе. Не забудьте зафиксировать этап перевода до выхода из системы!
...
* Бесцветный - Вы можете перевести эту строку в своём этапе перевода, но Вам не разрешено фиксировать в репозитории изменения для этого языка. Вы сможете только экспортировать свой этап перевода в файл.';
