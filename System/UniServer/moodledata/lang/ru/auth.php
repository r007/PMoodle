...
/**
 * Strings for component 'auth', language 'ru', branch 'MOODLE_22_STABLE'
 *
 * @package   auth
...
$string['alternatelogin'] = 'Если Вы введёте здесь адрес веб-страницы, то она будет использоваться в качестве страницы входа на сайт. Эта страница должна содержать форму, которая ведёт на <strong>\'{$a}\'</strong> и возвращает поля <strong>username</strong> и <strong>password</strong>.<br />Будьте внимательны, так как если Вы введёте здесь неправильный адрес, то заблокируете себе доступ к сайту.<br />Для использования стандартной страницы входа в систему оставьте это поле пустым.';
...
$string['auth_fieldlocks_help'] = '<p>Вы можете запретить пользователям редактировать некоторые поля профиля. Это полезно для сайтов, где данные пользователей редактируются администратором вручную или загружаются с  помощью команды "Загрузить пользователей". Если Вы блокируете поля, необходимые для работы Moodle, то убедитесь, что предоставляете всю информацию, необходимую для создания учётной записи, иначе учётная запись не будет работать.</p><p>Чтобы избежать подобных проблем, можно попробовать установить блокировку в режим "Разблокировано, если пусто"</p>';
...
$string['auth_multiplehosts'] = 'Можно указать несколько имён компьютеров ИЛИ IP-адресов, например "host1.ru;host2.ru;host3.ru" или "ххх.ххх.ххх.ххх;ххх.ххх.ххх.ххх"';
...
$string['auth_passwordisexpired'] = 'Ваш пароль устарел. Вы хотите сейчас поменять свой пароль?';
$string['auth_passwordwillexpire'] = 'Срок действия пароля истекает через {$a} дн. Вы хотите поменять свой пароль сейчас?';
$string['auth_remove_delete'] = 'Полностью удалить внутреннюю учётную запись';
$string['auth_remove_keep'] = 'Сохранить внутреннюю учётную запись';
$string['auth_remove_suspend'] = 'Заблокировать внутреннюю учётную запись';
$string['auth_remove_user'] = 'Укажите, что делать с внутренней учётной записью пользователя во время массовой синхронизации, если пользователь будет удален из внешнего источника. Только заблокированные пользователи могут быть автоматически восстановлены, если они снова появятся во внешнем источнике.';
$string['auth_remove_user_key'] = 'Удалён внешний пользователь';
$string['auth_sync_script'] = 'Скрипт синхронизации cron';
$string['auth_updatelocal'] = 'Обновление локальной учётной записи';
$string['auth_updatelocal_expl'] = '<p><b>Обновление локальной учётной записи:</b> если параметр включен, то поля будут обновляться (с внешним источником аутентификации) как при каждом входе пользователя, так и при синхронизации пользователей. Набор полей для локального обновления должен быть заблокирован.</p>';
$string['auth_updateremote'] = 'Внешнее обновление';
$string['auth_updateremote_expl'] = '<p><b>Внешнее обновление:</b> если параметр включен, то информация во внешнем источнике авторизации будет обновлена при обновлении пользовательских учётных записей. Поля должны быть разблокированы для разрешения изменений.</p>';
$string['auth_updateremote_ldap'] = '<p><b>Примечание:</b> Для обновление внешних данных в LDAP требует установки в binddn и bindpw пользователя с правами редактирования записей всех пользователей. В настоящее время при обновлении не сохраняются многозначные атрибуты и удаляются дополнительные значения.</p>';
$string['auth_user_create'] = 'Разрешить создание пользователей';
$string['auth_user_creation'] = 'Новые (анонимные) пользователи могут создавать учетные записи во внешнем источнике аутентификации и подтверждать их через электронную почту. Если Вы разрешите это, не забудьте также настроить опции конкретного модуля, необходимые для создания пользователей.';
...
$string['forgottenpassword'] = 'Если вы введёте здесь URL веб-страницы, она будет использоваться для восстановления забытого пароля сайта. Эта настройка предназначена для сайтов, где управление паролями осуществляется вне Moodle. Для использования стандартной процедуры восстановления пароля оставьте это поле пустым.';
...
$string['selfregistration_help'] = 'Если выбран такой плагин аутентификации, как самостоятельная регистрация по электронной почте, у потенциальных пользователей появляется возможность самостоятельно зарегистрироваться и создать учётную запись. Это может привести к тому, что спамеры начнут создавать учётные записи, чтобы использовать форумы, блоги и т.п. для распространения спама. Чтобы уменьшить риск, нужно отключить самостоятельную регистрацию или ограничить её, заполнив поле <em>Разрешенные почтовые домены</em>';
...
$string['stdchangepassword_explldap'] = 'ПРИМЕЧАНИЕ: рекомендуется использовать LDAP через зашифрованный SSL-туннель (ldaps://), если сервер LDAP установлен на удалённом компьютере.';
$string['suspended'] = 'Учётная запись заблокирована';
$string['suspended_help'] = 'Пользователи, чьи учётные запись заблокированы, не могут входить в систему или пользоваться веб-службами. Все исходящие сообщения для таких пользователей блокируются.';
