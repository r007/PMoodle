<?php //$Id$
    ...
    function import_backup_file_silently($pathtofile,$destinationcourse,$emptyfirst=false,$userdata=false, $preferences=array()) {
        ...
        $SESSION->restore->restore_site_files = 1;
        $SESSION->restore->updating = (isset($preferences->updating) ? $preferences->updating : 0);
    ...
    */
    ...
    /**
    * Function to generate the $preferences variable that
    * restore uses.  This will restore all modules and instances in a course.
    *
    * @param object $course course object
    * @param array $prefs can contain:
            restore_metacourse
   ...
   function restore_generate_preferences_artificially($course, $prefs) {
        global $CFG;
        $preferences = new StdClass;
        $count = 0;
        ...
        if ($allmods = get_records("modules") ) {
            foreach ($allmods as $mod) {
                ...
                foreach ($preferences->$varname as $instance) {
                    ...
                    $preferences->$var = true;
                    $preferences->mods[$modname]->instances[$instance->id]->userinfo = true;
                    $var = 'restore_'.$modname.'_instances';
                    $preferences->$var = 1; // we need this later to determine what to display in modcheckbackup.
                }
                ...
                $preferences->$var = true;
                $preferences->mods[$modname]->restore = true;
        ...
        //Check other parameters
        $preferences->restore_metacourse = (isset($prefs['restore_metacourse']) ? $prefs['restore_metacourse'] : 0);
        $preferences->restore_users = (isset($prefs['restore_users']) ? $prefs['restore_users'] : 0);
        $preferences->restore_logs = (isset($prefs['restore_logs']) ? $prefs['restore_logs'] : 0);
        $preferences->restore_user_files = (isset($prefs['restore_user_files']) ? $prefs['restore_user_files'] : 0);
        $preferences->restore_course_files = (isset($prefs['restore_course_files']) ? $prefs['restore_course_files'] : 0);
        $preferences->restore_site_files = (isset($prefs['restore_site_files']) ? $prefs['restore_site_files'] : 0);
        $preferences->restore_messages = (isset($prefs['restore_messages']) ? $prefs['restore_messages'] : 0);
        $preferences->restore_course = $course->id;
        return $preferences;
    }
    
    function backup_add_to_log($starttime,$courseid,$message,$backuptype) { 
        if (!$starttime && $backuptype='scheduledbackup') {
            //for some reason when the startime=0 we didn't store anything here for scheduledbackups - not sure why!
        } else {
            $log = new StdClass;
