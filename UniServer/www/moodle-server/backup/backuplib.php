<?php //$Id$
    ...
    function backup_general_info ($bf,$preferences) {
        ...
        if (!isset($preferences->backup_generate_incrementals) && !$preferences->backup_generate_incrementals) { //don't put lastaccess in the backup as this changes all the time and isn't really needed.
            //The date        
        ...
        //The blogs in backup
        if (isset($preferences->backup_blogs) && $preferences->backup_blogs == 1 && $preferences->backup_course == SITEID) {
    ...
    function backup_user_info ($bf,$preferences) {
        ...
        if ($users && !rs_EOF($users)) {
            ...
            while ($user = rs_fetch_next_record($users)) {
                ...
                if (!isset($preferences->backup_private_user_data) or !$preferences->backup_private_user_data) { //hide user data when backup_private_user_data is true.
    ...
    function backup_gradebook_info($bf, $preferences) {
        ...
        if (isset($preferences->backup_gradebook_history) && $preferences->backup_gradebook_history) {
    ...
    function copy_zip_to_course_dir ($preferences) {
        ...
        //insert record into backup_instance table with details on new backup.
        $bkinstance = new object();
        $bkinstance->courseid = $preferences->backup_course;
        $bkinstance->filename = $preferences->backup_name;
        $bkinstance->hash = md5($preferences->backup_name);
        $bkinstance->timecreated = time();
        if (!record_exists('incremental_instance', 'courseid', $preferences->backup_course, 'hash',  md5($preferences->backup_name))) {
    ...
    function backup_fetch_prefs_from_request(&$preferences,&$count,$course) {
        ...
        $preferences->backup_generate_incrementals = optional_param('backup_generate_incrementals', 0);
        $preferences->backup_private_user_data = optional_param('backup_private_user_data', 0);
    ...
    */
    ...
    function get_backup_guid($table, $field, $value) {
        global $CFG;
    
        if ($record = get_record('backup_guids', 'src_table', $table, 'src_field', $field, 'src_value', $value, 'id, guid')) {
            return($record->guid);
        } else {
            $backupguid = new stdClass();
            $backupguid->guid = sha1($CFG->siteidentifier.'-'.$table.'-'.$field.'-'.$value);
            $backupguid->src_table = $table;
            $backupguid->src_field = $field;
            $backupguid->src_value = $value;
        
            if(insert_record('backup_guids', $backupguid)) {
                return($backupguid->guid);
            } else {
                error('Failed to insert backup GUID');
            }
    }
