...
define('RESTORE_GROUPS_GROUPINGS', 3);
                                    ...
                                    if ($module) {
                                        ...
                                        if ($status) {
                                            if ($sequence) {
                                                ...
                                                if ($sequence===$newidmod or strpos($sequence, ','.$newidmod.',') 
                                                     or strpos($sequence, $newidmod.',')===0 
                                                     or strpos($sequence, ','.$newidmod)==(strlen($sequence)-strlen($newidmod)-1)) { //if both the same length then check like this:
        ...
        /// Iterate over all users loaded from xml
            ...
            foreach ($info->users as $userid) {
                ...
                /// Disable pictures based on global setting or existing empty value (old backups can contain wrong empties)
                    ...
                    if (isset($restore->original_wwwroot) && $restore->original_wwwroot != $CFG->wwwroot) {
                        $user->policyagreed = 0;
                    } else {
    ...
    function restore_group_getid($restore, $groupid) {
        ...
        if (!isset($restore->groups) or $restore->groups == RESTORE_GROUPS_NONE or $restore->groups == RESTORE_GROUPINGS_ONLY) {
            $group->new_id = 0;
        }
        return $group;
    }
    ...
    function restore_execute(&$restore,$info,$course_header,&$errorstr) {
        ...
        if ($status and (isset($restore->groups) and ($restore->groups == RESTORE_GROUPS_ONLY or $restore->groups == RESTORE_GROUPS_GROUPINGS))) {
            if (!defined('RESTORE_SILENTLY')) {
        ...
        if ($status and (isset($restore->groups) and ($restore->groups == RESTORE_GROUPINGS_ONLY or $restore->groups == RESTORE_GROUPS_GROUPINGS))) {
            if (!defined('RESTORE_SILENTLY')) {
        ...
        if ($status and isset($restore->groups) and $restore->groups == RESTORE_GROUPS_GROUPINGS) {
            if (!defined('RESTORE_SILENTLY')) {
        ...
        if ($status and isset($restore->blogs) and ($restore->blogs)) {
            if (!defined('RESTORE_SILENTLY')) {
...
/********************** Roles and Capabilities Related Functions *******************************/
    ...
    function restore_roles_settings($restore, $xmlfile) {
        ...
        if ((!empty($course->roleassignments) && !$isimport) or (!empty($course->roleassignments) && $isimport && isset($restore->users) && $restore->users)) {
    ...
    function get_id_from_backup_guid($guid, $courseid) {
    
    if ($record = get_record('restore_guids', 'guid', $guid, 'courseid', $courseid)) {
        return($record);
    } else {
        return(false);
    }
...
function save_restore_guid($guid, $table, $field, $value, $courseid) {

    $backupguid = new stdClass();
    $backupguid->guid = $guid;
    $backupguid->src_table = $table;
    $backupguid->src_field = $field;
    $backupguid->src_value = $value;
    $backupguid->courseid  = $courseid;
    
    if(insert_record('restore_guids', $backupguid)) {
        return($backupguid->guid);
    } else {
        error('Failed to insert restore GUID');
    }
...
//echo "<pre>\nDEBUG: ".print_r($restore, true)."\n</pre>";
    if (!isset($guid)) {
        ...
        if ($restoreto = get_id_from_backup_guid($guid, $restore->course_id)) {
            ...
            if ($recordexists = get_record($table, $field, $restoreto->src_value)) {
            ...
            } else { //record doesn't exist so need to insert it.
                ...
                return(false); //should return false as this is a failure!
            }
