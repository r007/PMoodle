...
//echo "<pre>\nDEBUG: ".print_r($restore, true)."\n</pre>";
        ...
        if ($data) {
            ...
            if (!$cmid = get_id_from_backup_guid($guid, $forum->course)) { //get course_module Id for this forum
    ...
    function forum_discussions_restore_mods($forum_id,$info,$restore) {
        ...
        for($i = 0; $i < sizeof($discussions); $i++) {
            ...
            $guid = backup_todb($dis_info['#']['GUID']['0']['#']);
            $newid = restore_with_guid($restore, 'forum_discussions', 'id', $discussion, $guid);
