...
function schedule_backup_cron() {
    ...
    if(!isset($backup_config->backup_sche_active) || !$backup_config->backup_sche_active) {
        ...
        $numofrec = count_records_select ("backup_log","time > $timeafter AND type='scheduledbackup'");
        if (!$numofrec) {
...
function schedule_backup_course_configure($course,$starttime = 0) {
    ...
    backup_add_to_log($starttime,$course->id,"    checking parameters", 'scheduledbackup');
    ...
    if ($status) {
        ...
        if (!isset($backup_config->backup_sche_incrementals)) {
            $backup_config->backup_sche_incrementals = 0;
        }
    }
    ...
    if ($status) {
        ...
        $preferences->backup_sche_incrementals = $backup_config->backup_sche_incrementals;
    }

    //Calculate various backup preferences
    if ($status) {
        backup_add_to_log($starttime,$course->id,"    calculating backup name", 'scheduledbackup');
    ...
    if ($status) {
        backup_add_to_log($starttime,$course->id,"    calculating modules data", 'scheduledbackup');
        if ($allmods = get_records("modules") ) {
            foreach ($allmods as $mod) {
                $modname = $mod->name;
                $modbackup = $modname."_backup_mods";
                ...
                if (isset($$var) && $$var) {
                    ...
                    if ($$var == 1) {
                        ...
                        $modcheckbackup = $modname."_check_backup_mods";
                        backup_add_to_log($starttime,$course->id,"      $modname", 'scheduledbackup');
    ...
    if ($status) {
        backup_add_to_log($starttime,$course->id,"    calculating users", 'scheduledbackup');
    ...
    if ($status) {
        if ($preferences->backup_logs) {
            backup_add_to_log($starttime,$course->id,"    calculating logs", 'scheduledbackup');
    ...
    if ($status) {
        if ($preferences->backup_user_files) {
            backup_add_to_log($starttime,$course->id,"    calculating user files", 'scheduledbackup');
    ...
    if ($status) {
       if ($preferences->backup_course_files) {
            backup_add_to_log($starttime,$course->id,"    calculating course files", 'scheduledbackup');
    ...
    if ($status) {
       if ($preferences->backup_site_files) {
            backup_add_to_log($starttime,$course->id,"    calculating site files", 'scheduledbackup');
...
function schedule_backup_course_execute($preferences,$starttime = 0) {
    ...
    backup_add_to_log($starttime,$preferences->backup_course,"    checking temp structures", 'scheduledbackup');
    $status = check_and_create_backup_dir($preferences->backup_unique_code);
    //Empty backup dir
    if ($status) {
        backup_add_to_log($starttime,$preferences->backup_course,"    cleaning current dir", 'scheduledbackup');
        $status = clear_backup_dir($preferences->backup_unique_code);
    }

    //Create the moodle.xml file
    if ($status) {
        backup_add_to_log($starttime,$preferences->backup_course,"    creating backup file", 'scheduledbackup');
        ...
        if ($backup_file) {
            backup_add_to_log($starttime,$preferences->backup_course,"      general info", 'scheduledbackup');
        ...
        if ($status && $preferences->backup_metacourse) {
            backup_add_to_log($starttime,$preferences->backup_course,"      metacourse info", 'scheduledbackup');
            $status = backup_course_metacourse($backup_file,$preferences);
        }

        //Block info
        if ($status) {
            backup_add_to_log($starttime,$preferences->backup_course,"      blocks info", 'scheduledbackup');
            $status = backup_course_blocks($backup_file,$preferences);
        }

        //Section info
        if ($status) {
            backup_add_to_log($starttime,$preferences->backup_course,"      sections info", 'scheduledbackup');
            $status = backup_course_sections($backup_file,$preferences);
        }

        //User info
        if ($status) {
            backup_add_to_log($starttime,$preferences->backup_course,"      user info", 'scheduledbackup');
        ...
        if ($status && $preferences->backup_messages && $preferences->backup_course == SITEID) {
            backup_add_to_log($starttime,$preferences->backup_course,"      messages", 'scheduledbackup');
        ...
        if ($status && $preferences->backup_blogs && $preferences->backup_course == SITEID) {
            backup_add_to_log($starttime,$preferences->backup_course,"      blogs", 'scheduledbackup');
        ...
        if ($status and $preferences->mods['quiz']->backup) {
            backup_add_to_log($starttime,$preferences->backup_course,"      categories & questions", 'scheduledbackup');
        ...
        if ($status) {
            if ($preferences->backup_logs) {
                backup_add_to_log($starttime,$preferences->backup_course,"      logs", 'scheduledbackup');
        ...
        //Print scales info
        if ($status) {
            backup_add_to_log($starttime,$preferences->backup_course,"      scales", 'scheduledbackup');
            $status = backup_scales_info($backup_file,$preferences);
        }

        //Print groups info
        if ($status) {
            backup_add_to_log($starttime,$preferences->backup_course,"      groups", 'scheduledbackup');
            $status = backup_groups_info($backup_file,$preferences);
        }

        //Print groupings info
        if ($status) {
            backup_add_to_log($starttime,$preferences->backup_course,"      groupings", 'scheduledbackup');
            $status = backup_groupings_info($backup_file,$preferences);
        }

        //Print groupings_groups info
        if ($status) {
            backup_add_to_log($starttime,$preferences->backup_course,"      groupings_groups", 'scheduledbackup');
            $status = backup_groupings_groups_info($backup_file,$preferences);
        }

        //Print events info
        if ($status) {
            backup_add_to_log($starttime,$preferences->backup_course,"      events", 'scheduledbackup');
            $status = backup_events_info($backup_file,$preferences);
        }

        //Print gradebook info
        if ($status) {
            backup_add_to_log($starttime,$preferences->backup_course,"      gradebook", 'scheduledbackup');
        ...
        if ($status) {
            ...
            if ($mods_to_backup) {
                backup_add_to_log($starttime,$preferences->backup_course,"      modules", 'scheduledbackup');
                //Start modules tag
                $status = backup_modules_start ($backup_file,$preferences);
                //Iterate over modules and call backup
                foreach ($preferences->mods as $module) {
                    if ($module->backup and $status) {
                        backup_add_to_log($starttime,$preferences->backup_course,"        $module->name", 'scheduledbackup');
        ...
        if ($status) {
            backup_add_to_log($starttime,$preferences->backup_course,"      course format data", 'scheduledbackup');
    ...
    if ($status) {
        if ($preferences->backup_user_files) {
            backup_add_to_log($starttime,$preferences->backup_course,"    copying user files", 'scheduledbackup');
    ...
    if ($status) {
        if ($preferences->backup_course_files) {
            backup_add_to_log($starttime,$preferences->backup_course,"    copying course files", 'scheduledbackup');
    ...
    if ($status) {
        if ($preferences->backup_site_files) {
            backup_add_to_log($starttime,$preferences->backup_course,"    copying site files", 'scheduledbackup');
    ...
    if ($status) {
        backup_add_to_log($starttime,$preferences->backup_course,"    zipping files", 'scheduledbackup');
        $status = backup_zip ($preferences);
    }

    //Now, copy the zip file to course directory
    if ($status) {
        backup_add_to_log($starttime,$preferences->backup_course,"    copying backup", 'scheduledbackup');
        $status = copy_zip_to_course_dir ($preferences);
    }

    //Now, clean temporary data (db and filesystem)
    if ($status) {
        backup_add_to_log($starttime,$preferences->backup_course,"    cleaning temp data", 'scheduledbackup');
...
function schedule_backup_course_delete_old_files($preferences,$starttime=0) {
    ...
    backup_add_to_log($starttime,$preferences->backup_course,"    checking $dirtocheck", 'scheduledbackup');
    if ($CFG->debug > 7) {
    ...
    backup_add_to_log($starttime,$preferences->backup_course,"        found $countmatching backup files", 'scheduledbackup');
    mtrace("                found $countmatching backup files");
    if ($preferences->backup_keep < $countmatching) {
        backup_add_to_log($starttime,$preferences->backup_course,"        keep limit ($preferences->backup_keep) reached. Deleting old files", 'scheduledbackup');
        ...
        foreach ($matchingfiles as $matchfile) {
            if ($filesdeleted < $filestodelete) {
                backup_add_to_log($starttime,$preferences->backup_course,"        $matchfile deleted", 'scheduledbackup');
